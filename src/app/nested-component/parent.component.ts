...
@Component({
    ...
    template: `
        <div class="row" [style.background-color]="getStyle()">부모<br>
            클릭수:{{cntClick}}<br>
            자식상태:{{active}}<br>
            <nested-child (outputproperty)="outputEvent($event)" (outputEventColor)="outputColor($event)" [name1]="fruit1" [name2]="fluit2()" [name3]="fluit3" [BColors]="backColors" [BCCodes]="backColorCodes">
            </nested-child>
...
export class NestedParentComponent {
    ...
    outputColor(colorNum:number){ //배경받아오기
